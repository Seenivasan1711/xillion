...
async def grid_search(
    ...
    for combo in combos:
        ...
        results.append(GridResult(params=params, metrics=result.metrics, trade_count=len(result.trades)))
...
class WalkForwardResult:
    ...
    avg_in_sample: Optional[float] = None
    avg_out_of_sample: Optional[float] = None
...
async def walk_forward(
    ...
    for i in range(n_folds):
        ...
        grid_results = await grid_search(
            strategy_cls, train_bars, instruments, timeframe, initial_capital,
            param_grid, base_params, slippage_bps, fee_config, rank_by,
        )
        if not grid_results:
            continue
        best = grid_results[0]
        ...
        test_result = await engine.run(
            strategy=strategy_cls(), bars=test_bars, instruments=instruments, timeframe=timeframe,
            initial_capital=initial_capital, params=best.params,
            slippage_bps=slippage_bps, fee_config=fee_config,
        )

        folds.append(WalkForwardFold(
            train_from=train_bars[0].ts, train_to=train_bars[-1].ts,
            test_from=test_bars[0].ts, test_to=test_bars[-1].ts,
            best_params=best.params, in_sample_metrics=best.metrics,
    ...
    is_likely_overfit = avg_in is not None and avg_in > 0 and (
        avg_out is None or avg_out <= 0 or avg_out < avg_in * 0.5
    )

    return WalkForwardResult(
        rank_by=rank_by, folds=folds,
        avg_in_sample=avg_in, avg_out_of_sample=avg_out,
