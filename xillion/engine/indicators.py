...
def vwap(bars: list[Bar], period: int) -> float | None:
...
def macd(
    ...
    macd_line_series = [f - s for f, s in zip(fast_ema, slow_ema, strict=False)]
...
def supertrend(
    bars: list[Bar], period: int = 10, multiplier: float = 3.0
    ...
    for i in range(1, len(bars)):
        ...
        if prev_upper is None:
            ...
            # prev_upper/prev_lower are always set together (line ~165), so
            # prev_upper being non-None here guarantees prev_lower is too --
            # mypy can't infer that correlation across two separate
            # Optional variables on its own.
