...
def _match_fills(rows: Sequence[Any]) -> list[dict[str, Any]]:
    """
    FIFO-match BUY and SELL fills into round-trip trades.

    Each row is (FillRecord, strategy_instance_id, instance_name, mode) --
    typed as Sequence[Any] rather than list[tuple] because callers pass
    SQLAlchemy Row objects, which support the same positional unpacking as
    a tuple but aren't statically typed as one.
    """
