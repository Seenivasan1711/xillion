...
def test_indicator_metric_threshold():
    ...
    assert (
        evaluate({"metric": {"name": "rsi", "period": 5}, "operator": ">", "threshold": 70}, bars)
        is True
    )
    assert (
        evaluate({"metric": {"name": "rsi", "period": 5}, "operator": ">", "threshold": 80}, bars)
        is False
    )
...
def test_evaluate_all_is_and_of_every_condition():
    ...
    conditions_with_one_false = conditions + [
        {"metric": {"name": "close"}, "operator": ">", "threshold": 100}
    ]
