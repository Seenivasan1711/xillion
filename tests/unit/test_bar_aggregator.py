...
def test_bucket_start_rounds_down_to_the_timeframe():
    ts = datetime(2026, 1, 1, 9, 17, 43, tzinfo=timezone.utc)
    assert _bucket_start(ts, 300) == datetime(2026, 1, 1, 9, 15, 0, tzinfo=timezone.utc)  # 5m
    assert _bucket_start(ts, 60) == datetime(2026, 1, 1, 9, 17, 0, tzinfo=timezone.utc)   # 1m
...
async def test_bar_publishes_with_correct_ohlc_when_bucket_closes():
    ...
    await agg.on_tick(_tick("NIFTY", "200", START + timedelta(minutes=5)))   # first tick of bucket 2 -> closes bucket 1
...
async def test_volume_is_the_cumulative_delta_not_a_sum_of_raw_ticks():
    ...
    await agg.on_tick(_tick("NIFTY", "103", START + timedelta(minutes=5), volume=12_000))  # closes bucket 1
...
async def test_multiple_timeframes_for_the_same_symbol_aggregate_independently():
    ...
    await agg.on_tick(_tick("NIFTY", "100", START))
    await agg.on_tick(_tick("NIFTY", "101", START + timedelta(minutes=6)))   # closes the 5m bucket only
    assert len(published_5m) == 1
    assert len(published_15m) == 0

    await agg.on_tick(_tick("NIFTY", "102", START + timedelta(minutes=16)))  # closes the 15m bucket too
...
async def test_a_late_out_of_order_tick_does_not_reopen_a_closed_bucket():
    ...
    await agg.on_tick(_tick("NIFTY", "100", START))
    await agg.on_tick(_tick("NIFTY", "200", START + timedelta(minutes=5)))  # closes bucket 1 at close=100
    await agg.on_tick(_tick("NIFTY", "999", START + timedelta(minutes=1)))  # late tick, belongs to bucket 1
