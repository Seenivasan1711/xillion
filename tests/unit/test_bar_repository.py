...
async def test_upsert_and_get_round_trip():
    ...
    bars = await repo.get_bars("NIFTY_TEST_RT", "1d", datetime(2026, 7, 1), datetime(2026, 9, 1), exchange="NFO")
    assert len(bars) == 1
...
async def test_upsert_is_idempotent_and_updates_on_conflict():
    ...
    await repo.upsert_bars([_bar("NIFTY_TEST_CONFLICT", ts, "150")], exchange="NFO")  # same PK, new close

    bars = await repo.get_bars("NIFTY_TEST_CONFLICT", "1d", datetime(2026, 7, 1), datetime(2026, 9, 1), exchange="NFO")
...
async def test_get_bars_filters_by_exchange():
    ...
    nse_bars = await repo.get_bars("SAME_SYMBOL_DIFF_EXCH", "1d", datetime(2026, 7, 1), datetime(2026, 9, 1), exchange="NSE")
    nfo_bars = await repo.get_bars("SAME_SYMBOL_DIFF_EXCH", "1d", datetime(2026, 7, 1), datetime(2026, 9, 1), exchange="NFO")
...
async def test_upsert_batches_large_row_counts():
    ...
    repo = BarRepository(get_session_factory())
    ts = datetime(2026, 8, 1)
    bars = [_bar(f"BULK_SYM_{i}", ts, str(i)) for i in range(350)]  # well over one SQLite statement's worth

    await repo.upsert_bars(bars, exchange="NFO")

    got = await repo.get_bars("BULK_SYM_200", "1d", datetime(2026, 7, 1), datetime(2026, 9, 1), exchange="NFO")
    assert len(got) == 1
