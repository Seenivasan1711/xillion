    ...
    async with factory() as session:
        session.add(
            StrategyInstance(
                ...
                name="Test Instance For Signals API",
                mode="alert",
                status="running",
    ...
    async with factory() as session:
        session.add(
            SignalLog(
                ...
                stop_loss_price=24950.0,
                side="BUY",
                price=25000.0,
                message="BUY ENTER: NIFTY 50",
                mode="alert",
                notified=True,
                notified_at=_now(),
            )
...
async def test_join_query_resolves_instance_name_and_filters_by_instance(monkeypatch):
    ...
    async with factory() as session:
        session.add(
            SignalLog(
                strategy_instance_id=instance_a,
                ts=_now(),
                underlying_symbol="NIFTY 50",
                signal_type="SIGNAL",
                side="BUY",
        ...
        session.add(
            SignalLog(
                strategy_instance_id=instance_b,
                ts=_now(),
                underlying_symbol="NIFTY 50",
                signal_type="SIGNAL",
                side="BUY",
    ...
    async with factory() as session:
        stmt = (
            ...
            .join(
                StrategyInstance,
                SignalLog.strategy_instance_id == StrategyInstance.id,
                isouter=True,
            )
