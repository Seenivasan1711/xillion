...
from xillion.core.multileg import (
    ...
    credit_adequate, max_loss_per_lot, order_entry_sequence, order_exit_sequence,
    size_defined_risk_position,
)


def _spread_spec(short_offset=None) -> MultiLegSpec:
    long_leg = Leg(symbol="NIFTY_LONG", exchange="NFO", role=LegRole.LONG,
                   side=Side.BUY, quantity=65, order_type=OrderType.MARKET)
    short_leg = Leg(symbol="NIFTY_SHORT", exchange="NFO", role=LegRole.SHORT,
                    side=Side.SELL, quantity=65, order_type=OrderType.MARKET,
                    protects_leg_index=1)
    return MultiLegSpec(
        structure_type=StructureType.CREDIT_SPREAD, underlying="NIFTY",
        legs=[short_leg, long_leg], lot_size=65,  # deliberately out of order
        width=Decimal("50"), credit=Decimal("10"),
    )
...
def test_spec_rejects_short_leg_protecting_a_nonexistent_long():
    bad_short = Leg(symbol="X", exchange="NFO", role=LegRole.SHORT, side=Side.SELL,
                     quantity=65, protects_leg_index=5)
    with pytest.raises(ValueError):
        MultiLegSpec(structure_type=StructureType.CREDIT_SPREAD, underlying="NIFTY",
                     legs=[bad_short], lot_size=65)


def test_entry_sequence_puts_long_first_regardless_of_list_order():
    spec = _spread_spec()
    ordered = order_entry_sequence(spec)
    assert [l.role for l in ordered] == [LegRole.LONG, LegRole.SHORT]


def test_exit_sequence_puts_short_first():
    spec = _spread_spec()
    ordered = order_exit_sequence(spec)
    assert [l.role for l in ordered] == [LegRole.SHORT, LegRole.LONG]


# ── Sizing (KB 10 §7 worked example, Rs3,00,000 capital @ 1% risk) ─────────

def test_max_loss_per_lot_credit_spread_nifty_200_wide():
    loss = max_loss_per_lot(StructureType.CREDIT_SPREAD, 65, width=Decimal("200"), credit=Decimal("30"))
...
def test_size_nifty_50_wide_is_one_lot():
    loss = max_loss_per_lot(StructureType.CREDIT_SPREAD, 65, width=Decimal("50"), credit=Decimal("10"))
...
def test_size_sensex_100_wide_is_one_lot():
    loss = max_loss_per_lot(StructureType.CREDIT_SPREAD, 20, width=Decimal("100"), credit=Decimal("18"))
...
def test_size_respects_max_lots_cap():
    decision = size_defined_risk_position(Decimal("10000000"), Decimal("0.01"), Decimal("100"), max_lots_cap=5)
