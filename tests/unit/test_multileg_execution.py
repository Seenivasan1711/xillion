...
def _order(request: OrderRequest, status: OrderStatus, filled_qty: int = 0, price=None) -> Order:
    return Order(
        ...
        submitted_at=_now(), updated_at=_now(), broker_order_id=f"B-{uuid4().hex[:6]}",
        filled_quantity=filled_qty, avg_fill_price=price if filled_qty else None,
    )
...
def _two_leg_spec(short_symbol="SHORT", long_symbol="LONG") -> MultiLegSpec:
    long_leg = Leg(symbol=long_symbol, exchange="NFO", role=LegRole.LONG,
                   side=Side.BUY, quantity=65, order_type=OrderType.MARKET)
    short_leg = Leg(symbol=short_symbol, exchange="NFO", role=LegRole.SHORT,
                    side=Side.SELL, quantity=65, order_type=OrderType.MARKET,
                    protects_leg_index=0)
    return MultiLegSpec(structure_type=StructureType.CREDIT_SPREAD, underlying="NIFTY",
                        legs=[long_leg, short_leg], lot_size=65,
                        width=Decimal("50"), credit=Decimal("10"))


def _executor(broker: FakeBroker, **kwargs) -> MultiLegExecutor:
    return MultiLegExecutor(
        place_order_fn=broker.place_order, cancel_order_fn=broker.cancel_order,
        get_order_fn=broker.get_order, fill_timeout_sec=0.3, poll_interval_sec=0.05, **kwargs,
    )
...
async def test_short_fills_without_its_protecting_long_forces_unwind():
    ...
    fake_fill_order = _order(
        OrderRequest(symbol=short_leg.symbol, side=short_leg.side, quantity=short_leg.quantity,
                    order_type=short_leg.order_type),
        OrderStatus.FILLED, short_leg.quantity, price=Decimal("10"),
    )
    result = await executor._rollback(spec, [LegFill(leg=short_leg, order=fake_fill_order)], [spec.legs[0]], False)
...
async def test_exit_places_short_before_long_and_reverses_sides():
    ...
    assert result.outcome == ExecutionOutcome.SUCCESS
    assert broker.placed[0].symbol == "SHORT" and broker.placed[0].side == Side.BUY  # buy-to-close short first
    assert broker.placed[1].symbol == "LONG" and broker.placed[1].side == Side.SELL  # sell-to-close long second
