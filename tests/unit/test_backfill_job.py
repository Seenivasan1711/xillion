...
class _FakeProvider(HistoricalDataProvider):
    ...
    async def fetch_bars(self, symbol, exchange, timeframe, from_date, to_date, **kw):
        return [Bar(
            symbol=symbol, timeframe=timeframe, ts=datetime.combine(from_date, datetime.min.time()),
...
def _app_state(provider_cls):
    return SimpleNamespace(
        ...
        plugin_loader=SimpleNamespace(registry=SimpleNamespace(data_providers={provider_cls.name: provider_cls})),
    )
...
async def test_successful_backfill_job_reports_done_with_bar_count():
    ...
    body = BackfillRequest(
        provider_name=_FakeProvider.name, symbol="BACKFILL_JOB_SYM", exchange="NFO",
        timeframe="1d", from_date=date(2026, 8, 3), to_date=date(2026, 8, 3),
    )
...
async def test_failing_provider_reports_failed_status_not_an_unhandled_exception():
    await init_db()
    body = BackfillRequest(
        provider_name=_FailingProvider.name, symbol="BACKFILL_JOB_SYM_2", exchange="NFO",
        timeframe="1d", from_date=date(2026, 8, 3), to_date=date(2026, 8, 3),
    )
