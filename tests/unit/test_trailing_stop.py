...
def _long_state(entry=100, stop=90, hwm=100, risk=10) -> TrailState:
    return TrailState(
        direction=TrailDirection.LONG,
        entry_price=Decimal(str(entry)),
        stop=Decimal(str(stop)),
        high_water_mark=Decimal(str(hwm)),
        low_water_mark=Decimal(str(entry)),
        initial_risk_per_unit=Decimal(str(risk)),
    )


def _short_state(entry=100, stop=110, lwm=100, risk=10) -> TrailState:
    return TrailState(
        direction=TrailDirection.SHORT,
        entry_price=Decimal(str(entry)),
        stop=Decimal(str(stop)),
        high_water_mark=Decimal(str(entry)),
        low_water_mark=Decimal(str(lwm)),
        initial_risk_per_unit=Decimal(str(risk)),
    )
...
def test_ratchet_short_takes_the_lower_value():
    ...
    assert ratchet(TrailDirection.SHORT, Decimal("110"), Decimal("115")) == Decimal(
        "110"
    )  # rejected
...
def test_credit_trail_does_nothing_below_capture_threshold():
    ...
    candidate = credit_trail(
        state, credit_received=Decimal("25"), current_spread_value=Decimal("17.5")
    )
...
def test_credit_trail_tightens_past_capture_threshold():
    ...
    candidate = credit_trail(
        state, credit_received=Decimal("25"), current_spread_value=Decimal("10")
    )
...
def test_credit_trail_never_loosens_even_if_spread_value_bounces_back_up():
    ...
    candidate = credit_trail(
        state, credit_received=Decimal("25"), current_spread_value=Decimal("15")
    )
...
def test_credit_trail_zero_credit_is_a_noop():
    ...
    assert (
        credit_trail(state, credit_received=Decimal("0"), current_spread_value=Decimal("5"))
        == state.stop
    )
...
def test_breakeven_shift_returns_none_once_already_moved():
    state = TrailState(
        ...
        low_water_mark=Decimal("100"),
        initial_risk_per_unit=Decimal("10"),
