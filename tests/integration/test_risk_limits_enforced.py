...
async def _spawn(engine, bus, instance_id, risk_limits, capital="100000"):
    ...
    return await engine.spawn(
        instance_id=instance_id, strategy_name=_AlwaysBuyStrategy.name,
        broker=DummyBroker(), instruments=["NIFTY"], timeframe="1m",
        capital=Decimal(capital), params={}, mode="paper",
...
async def test_max_open_positions_from_ui_config_actually_rejects_real_orders():
    ...
    order1 = await runner._ctx.place_order(OrderRequest(
    ...
    runner._ctx._positions["NIFTY"] = Position(
        symbol="NIFTY", quantity=1, avg_price=Decimal("100"), realised_pnl=Decimal("0"),
        unrealised_pnl=Decimal("0"), last_price=Decimal("100"),
    )
    order2 = await runner._ctx.place_order(OrderRequest(
        symbol="BANKNIFTY", side=Side.BUY, quantity=1, order_type=OrderType.MARKET,
    ))
...
async def test_hot_reload_tightens_limit_on_a_running_instance_without_restart():
    ...
    order2 = await runner._ctx.place_order(OrderRequest(
        symbol="FINNIFTY", side=Side.BUY, quantity=1, order_type=OrderType.MARKET,
    ))
    assert order2.status.value == "REJECTED"
    assert "positions" in order2.rejection_reason
