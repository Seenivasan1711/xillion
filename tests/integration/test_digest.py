...
async def _seed_broker_connection(db) -> int:
    ...
    unique = uuid4().hex
    bc = BrokerClass(
        name=f"Digest Test Broker {unique}",
        module_path="x",
        class_name="X",
        version="1.0.0",
        capabilities_json="{}",
...
async def _seed_instance(
    db, instance_id: str, name: str, broker_connection_id: int, status: str = "running"
) -> None:
    db.add(
        StrategyInstance(
            id=instance_id,
...
async def test_build_digest_computes_real_pnl_from_fills():
    ...
    async with factory() as db:
        ...
        await _seed_round_trip(
            db, "digest-t1", "digest-inst-1", conn_id, "NIFTY", "100", "110", 10, datetime.now(UTC)
        )
        # Losing trade: bought at 100, sold at 95, qty 10 -> -50
        await _seed_round_trip(
            db, "digest-t2", "digest-inst-1", conn_id, "NIFTY", "100", "95", 10, datetime.now(UTC)
        )
        await db.commit()
...
async def test_build_digest_excludes_trades_outside_the_period():
    ...
    async with factory() as db:
        ...
        await _seed_round_trip(
            db, "digest-old", "digest-inst-2", conn_id, "NIFTY", "100", "200", 1, old_ts
        )
        await db.commit()
...
def test_format_digest_message_with_trades_and_errors():
    ...
    report = DigestReport(
        ...
        error_count=4,
        running_instances=["Strat A"],
        errored_instances=["Strat B"],
    )
    msg = format_digest_message(report)
