...
async def _seed_instance(instance_id: str) -> None:
    ...
    async with factory() as db:
        ...
        sc = StrategyClass(
            ...
            name=f"{_StatefulStrategy.name} ({instance_id})", module_path="x", class_name="X",
            version="1.0.0", params_schema_json="{}", code_hash="abc",
...
def _spawn_kwargs(instance_id: str, restored_state=None) -> dict:
    ...
    return dict(
        instance_id=instance_id, strategy_name=_StatefulStrategy.name,
...
def _bar(symbol="NIFTY", close="24100") -> Bar:
    ...
    return Bar(symbol=symbol, timeframe="5m", ts=now, open=Decimal(close), high=Decimal(close),
