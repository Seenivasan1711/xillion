...
def load_mt5(symbol, months, offset):
    try:
        ...
        sys.exit("MetaTrader5 not installed.  pip install MetaTrader5\n"
    ...
    if mt5.symbol_info(symbol) is None:
        avail = [s.name for s in mt5.symbols_get()
    ...
    if not frames:
        sys.exit(f"No M1 history for {symbol}.  Open an M1 chart in MT5 and "
...
def load_csv(path, offset=0):
    ...
    tcol = next((c for c in df.columns
                 if c in ("time", "date", "datetime", "timestamp")), None)
    if tcol is None:
...
def check_timezone(m1, offset):
    ...
    if 20 <= quiet <= 23 or quiet == 0:
        ...
        print(f"     Server offset may be wrong. Try --offset "
              f"{(offset + quiet - 22) % 24}")

def resample_m5(m1):
    return (m1.resample("5min")
...
def resolve(eidx, entry, sl, direction, m1, tps):
    ...
    for i in range(eidx, end):
        ...
        if stopped:
            ...
            return dict(mfe=mfe, mae=mae, sl_hit=True, sl_time=idx[i],
                        timeout_price=np.nan, timeout_time=pd.NaT,
                        tp_time=tp_time, tp_mae=tp_mae)
    ...
    j = max(end - 1, eidx)
    return dict(mfe=mfe, mae=mae, sl_hit=False, sl_time=pd.NaT,
                timeout_price=cl_a[j], timeout_time=idx[j],
                tp_time=tp_time, tp_mae=tp_mae)
            ...
            for name, (level, kind) in lines.items():
                ...
                if poked:
                    ...
                    if name in pending:
                        ...
                        pending[name] = (pbi, max(pext, ext) if kind == "res"
                                         else min(pext, ext))
                    else:
                        pending[name] = (bi, ext)
                ...
                entry = (row["close"] - SPREAD_PTS if direction == "long"
                         else row["close"] + SPREAD_PTS)
                ...
                trades.append(Trade(
                    date=str(date.date()), weekday=date.strftime("%a"),
                    month=date.strftime("%Y-%m"), session=session,
                    line=name, direction=direction,
                    entry_time=t + pd.Timedelta(minutes=5),
                    entry=round(entry, 2), sl=round(sl, 2),
                    sl_pts=round(sl_pts, 2),
                    sl_hit=r["sl_hit"], sl_time=r["sl_time"],
                    mfe_pts=round(r["mfe"], 2), mae_pts=round(r["mae"], 2),
                    timeout_price=r["timeout_price"],
                    timeout_time=r["timeout_time"],
                    tp_time=r["tp_time"], tp_mae=r["tp_mae"],
                ))
                taken += 1
...
def evaluate(trades, tp):
    ...
    for tr in trades:
        if tp in tr.tp_time:
            ...
            move = ((tr.timeout_price - tr.entry) if tr.direction == "long"
        ...
        d = asdict(tr)
        d.pop("tp_time"); d.pop("tp_mae")
        d.update(tp_pts=tp, outcome=outcome, exit_time=xt,
                 exit_price=round(float(xp), 2), net_pts=round(net, 2),
                 pnl=round(net * USD_PER_PT - COMMISSION_USD, 2),
                 heat_pts=round(heat, 2),
                 heat_usd=round(heat * USD_PER_PT, 2),
                 hold_min=(int((xt - tr.entry_time).total_seconds() // 60)
                           if pd.notna(xt) else np.nan))
        rows.append(d)
...
def save_sqlite(trades_df, runs_row, path):
    ...
    try:
        pd.DataFrame([runs_row]).to_sql("runs", con, if_exists="append",
                                        index=False)
        t = trades_df.copy()
        ...
        con.execute("CREATE INDEX IF NOT EXISTS ix_trades_session "
                    "ON trades(session, month)")
        con.commit()
...
def report(trades, session, months):
    ...
    print("=" * 72)
    print(f"  {s['trades']} trades over {s['trading_days']} days   |   "
          f"win {s['win_rate']}%   |   expectancy ${s['expectancy']}")
    print(f"  Total ${s['total']}  ->  ${s['per_month']}/month   |   "
          f"final equity ${s['final']}"
          f"{'   ** BREACHED **' if s['breached'] else ''}")
    print(f"  Max DD ${s['max_dd']}   |   worst losing streak {s['worst_streak']}"
          f"   |   avg hold {s['avg_hold']} min   |   avg heat {s['avg_heat']} pts")

    print(f"\n  Prop rule check")
    warn = "   <-- 4 of these closes the account" if s['strikes'] >= 4 else ""
    print(f"    Trades with heat >= ${STRIKE_FLOATING_USD:.0f} "
          f"(Striking System): {s['strikes']}{warn}")
    print(f"    Days past the ${DAILY_LOSS_LIMIT:.0f} daily cap: {s['cap_days']}")
    print(f"    ${MAX_LOSS_FLOOR:.0f} equity floor: "
          f"{'HIT' if s['breached'] else 'never touched'}")

    print(f"\n  Month by month")
    print(f"    {'Month':<9}{'N':>5}{'Win%':>8}{'P&L $':>11}"
          f"{'Equity $':>12}{'MaxDD $':>10}")
    print("    " + "-" * 55)
    for m, g in base.groupby("month"):
        e = g["equity"]
        print(f"    {m:<9}{len(g):>5}{100 * (g['pnl'] > 0).mean():>8.1f}"
              f"{g['pnl'].sum():>11.2f}{e.iloc[-1]:>12.2f}"
              f"{(e.cummax() - e).max():>10.2f}")

    print(f"\n  TP sweep")
    print(f"    {'TP':>5}{'Win%':>8}{'Exp $':>9}{'/month $':>11}"
    ...
    for tp in TP_SWEEP:
        ...
        print(f"    {tp:>5.1f}{ss['win_rate']:>8.1f}{ss['expectancy']:>9.2f}"
              f"{ss['per_month']:>11.2f}{ss['max_dd']:>10.2f}"
              f"{ss['worst_streak']:>8}{ss['strikes']:>9}")
    print(f"\n    Best expectancy: TP {best[0]} pts (${best[1]}/trade)")
    print(f"    Only move the TP there if max DD stays under "
          f"${START_EQUITY - MAX_LOSS_FLOOR:.0f} and you can sit the streak.")

    print(f"\n  By level                          By weekday")
    lv = [(k, len(g), 100 * (g['pnl'] > 0).mean(), g['pnl'].mean())
          for k, g in base.groupby("line")]
    wd = [(k, len(g), 100 * (g['pnl'] > 0).mean(), g['pnl'].mean())
    ...
    for i in range(max(len(lv), len(wd))):
        a = (f"    {lv[i][0]:<11}n={lv[i][1]:<4}{lv[i][2]:>5.1f}% ${lv[i][3]:>6.2f}"
             if i < len(lv) else "")
        b = (f"   {wd[i][0]:<4}n={wd[i][1]:<4}{wd[i][2]:>5.1f}% ${wd[i][3]:>6.2f}"
             if i < len(wd) else "")
        print(f"{a:<38}{b}")
...
def main():
    ...
    p.add_argument("--offset", type=int, default=SERVER_UTC_OFFSET,
    ...
    p.add_argument("--db", default="sweep.db",
                   help="SQLite file to append this run to; '' to skip")
    a = p.parse_args()
    ...
    if len(m1) < 5000:
        sys.exit(f"Only {len(m1)} M1 bars - not enough. Scroll back an M1 "
                 f"chart in MT5 to force the download, then retry.")

    span = max((m1.index.max() - m1.index.min()).days / 30.44, 0.1)
    print(f"  {len(m1):,} M1 bars   {m1.index.min():%Y-%m-%d} -> "
    ...
    if not frames:
        print("\nNo trades in any window. Check --offset, or loosen "
              "SWEEP_LOOKBACK_BARS.")
        return

    print(f"\n{'=' * 72}\n  SESSION COMPARISON (TP {TP_PTS})\n{'=' * 72}")
    print(f"  {'Session':<10}{'N':>6}{'Win%':>8}{'Exp $':>9}"
          f"{'/month $':>11}{'MaxDD $':>10}{'Strikes':>9}")
    print("  " + "-" * 63)
    for df in frames:
        s = summarise(df, span)
        print(f"  {df['session'].iat[0]:<10}{s['trades']:>6}{s['win_rate']:>8.1f}"
              f"{s['expectancy']:>9.2f}{s['per_month']:>11.2f}"
              f"{s['max_dd']:>10.2f}{s['strikes']:>9}")

    out = pd.concat(frames, ignore_index=True)
    cols = ["session", "date", "weekday", "month", "line", "direction",
            "entry_time", "entry", "sl", "sl_pts", "exit_time", "exit_price",
            "outcome", "hold_min", "net_pts", "pnl", "equity",
            "mfe_pts", "heat_pts", "heat_usd"]
    out[cols].to_csv(a.out, index=False)
    print(f"\n  {len(out)} rows -> {a.out}")

    if a.db:
        overall = summarise(pd.concat(
            [f for f in frames if f["session"].iat[0] == "Both"] or frames[:1],
            ignore_index=True), span)
        run = dict(
            run_id=datetime.now().strftime("%Y%m%d-%H%M%S"),
            run_at=datetime.now().isoformat(timespec="seconds"),
            symbol=SYMBOL, months=round(span, 2),
            data_from=str(m1.index.min()), data_to=str(m1.index.max()),
            bars=len(m1), days=len(levels),
            tp_pts=TP_PTS, min_sl_pts=MIN_SL_PTS,
            sl_buffer_pts=SL_BUFFER_PTS, lookback_bars=SWEEP_LOOKBACK_BARS,
            max_trades_day=MAX_TRADES_PER_DAY, max_hold_min=MAX_HOLD_MIN,
            lot=LOT, spread_pts=SPREAD_PTS, commission=COMMISSION_USD,
            start_equity=START_EQUITY, server_offset=a.offset,
            trades=overall.get("trades"), win_rate=overall.get("win_rate"),
            expectancy=overall.get("expectancy"),
            per_month=overall.get("per_month"), max_dd=overall.get("max_dd"),
            worst_streak=overall.get("worst_streak"),
            strikes=overall.get("strikes"), breached=overall.get("breached"),
        )
        nr, nt = save_sqlite(out[cols], run, a.db)
        print(f"  appended to {a.db}  (run {run['run_id']}; "
              f"{nr} runs, {nt} trades stored)")

    print(f"  mfe_pts is what tells you later whether a bigger TP is justified.")
