...
- [ ] **Upload local backups + secrets to Drive, for a clean new-laptop
      setup.** Nothing in git covers this — a fresh `git clone` alone
      would leave a new machine missing real data and unable to log into
      anything. Four things, in priority order:
      1. **`.env`** (repo root) — every credential the app needs
         (Supabase `DATABASE_URL`, `MONGODB_URI`, `NOTION_API_TOKEN`,
         `TWELVE_DATA_API_KEY`, `FINNHUB_API_KEY`, `TELEGRAM_BOT_TOKEN`,
         `ZERODHA_PRIMARY_*` — real broker login + TOTP secret,
         `ENCRYPTION_KEY`, `APP_SECRET_KEY`). **Do not drop this in a
         plain/shared Drive folder as-is** — it's real broker credentials
         and API keys in plaintext. Either a password manager (1Password,
         Bitwarden) or an encrypted archive (`zip -e`/`7z`) in a
         Drive folder shared with no one, not a bare `.env` upload.
      2. **`data/.encryption_key`** — decrypts credentials stored via the
         Settings UI (e.g. DB-configured Twelve Data/Finnhub keys). Without
         this file, those DB-stored values are unreadable on a new machine
         even though the DB itself (Supabase) is cloud-hosted and
         reachable from anywhere. Same handling caution as `.env`.
      3. **XAUUSD M1 backfill** (Track B) — run `make backup-xauusd-research`,
         upload the resulting `.tar.gz` from
         `research/xauusd_scalping/data/backups/`. This is the ~23-hour
         Dukascopy backfill (174,554 M1 bars, 2026-03-01→2026-09-16) — by
         far the most expensive thing to reproduce if lost. Not secret,
         safe to store anywhere.
      4. **NSE warehouse DB** — run `make backup-warehouse`, upload the
         resulting snapshot from `data/backups/warehouse/`. Also not
         secret, safe to store anywhere, but currently ~1.6GB uncompressed
         (the backup script gzips it).
      **Not needed** — safe to skip: `data/xillion.db` (0 bytes, a known
      artifact of forgetting to export `DATABASE_URL` before running
      `alembic` locally, not real data — see CLAUDE.md's "Operational
      gotchas"), `data/test.db` (test fixture, regenerable),
      `frontend/dist/` and `research/xauusd_scalping/_backtest_results_raw.json`
      (build/analysis artifacts, regenerated by rerunning the relevant
      command). Supabase Postgres, MongoDB Atlas, and Notion are all
      cloud-hosted already — nothing to back up for them beyond the
      credentials in `.env` (item 1) needed to reconnect.
      **Blocks:** continuing this project from a new laptop without
      redoing ~23 hours of backfill or losing broker/API access.
      **Cost:** free (just your own Drive storage).
