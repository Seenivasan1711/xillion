...
- [x] **Uploaded local backups + secrets to Drive — done 2026-09-23.**
      ...
      M1 backfill archive (`make backup-xauusd-research`), and the NSE
      warehouse DB snapshot (`make backup-warehouse`). A new laptop can
      now pick up this project without redoing the ~23-hour Dukascopy
      backfill or losing broker/API credentials.
