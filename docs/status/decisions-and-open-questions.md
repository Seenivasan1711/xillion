...
### D28. Found `VolBucket` is dead code in the cost model — defer the fix to the real walk-forward harness build, don't fix in isolation
- **Decision:** confirmed `engine/cost_model.py`'s `VolBucket` classification is never actually used — `entry_cost_pts`/`exit_cost_pts` are called with `VolBucket.MEDIUM` hardcoded at both call sites in `backtest_engine.py` (`_open_position`, `_close_position`), so the harness's documented "session×volatility-bucket cost model" has only ever varied cost by session, never by realized volatility, in every run this project has done (v1-v3, C1, C2). Leaving this unfixed for now rather than patching it immediately and re-running everything again.
- **Why:** fixing it in isolation would immediately invalidate every comparison made so far (v1 vs v2 vs v3, real-cost vs zero-cost, M1 vs M5/M15) and require yet another full ~15-40 minute rerun cycle just to get back to a consistent baseline — and the walk-forward/holdout harness (still not built, see `04_plan.md` and 03_results.md's C2 section) is already a known, higher-priority prerequisite before any of these numbers should be treated as final. Bundling this fix into that build means one clean rerun instead of two.
- **Trade-off:** every cost figure produced by this project through 2026-09-23 slightly misrepresents true cost (likely understating it in genuinely high-volatility windows, overstating it in genuinely calm ones) — real but was constant across every comparison actually made, so it doesn't change any conclusion reached so far, only the absolute $ numbers' precision.
